...
function ParkingRoute() {
    ...
    const fetchParkingData = async () => {
        ...
        for (const region of ALL_REGIONS) {
            try {
            const response = await fetch(`https://testing-ne5w.onrender.com/parking_info/${region}`);
            ...
            allResults.push(...withCoordinates.filter(Boolean));

            } catch (err) {
            console.error(`${region} 데이터 불러오기 실패:`, err);
            }
        }

    setParkingLots(allResults);
    };
